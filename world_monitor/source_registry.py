...
SOURCE_SPECS: tuple[SourceSpec, ...] = (
    ...
    SourceSpec(
        ...
        notes="Historical base-rate context. The provider now requires a free access token.",
        operator_action="configure_provider_issued_ucdp_access_token",
    ),
    SourceSpec(
        "gdelt",
        "GDELT Project API",
    ...
    SourceSpec(
        ...
        "Read-only: four declared corridors rotate on the three-hour cadence. Raw detections are archived; only high-confidence/high-FRP anomalies become event evidence.",
    ),
    SourceSpec(
        "aviationstack",
    ...
    SourceSpec(
        ...
        ("https://services9-nocdn.arcgis.com/RHVPKKiFTONKtxq3/ArcGIS/rest/services/Active_Hurricanes_Sampler/FeatureServer/1/query",),
        "daily",
        "public layers no limit",
        status="adapter_live_optional",
        notes="Observed NOAA/Esri cyclone records only; not a claim of USACE infrastructure coverage.",
    ),
    SourceSpec(
    ...
    SourceSpec(
        ...
        "daily",
        "bounded daily public CSV; no undocumented API",
        status="adapter_live_optional",
    ...
    SourceSpec(
        ...
        "none",
        ("https://stats.bis.org/api/v1/data/WS_CBPOL/M.US/all",),
        "weekly",
        "no documented limit",
        status="adapter_live_optional",
        notes="Read-only US monthly policy-rate observations; reference periods are not publication timestamps.",
    ),
    SourceSpec(
        "ecb",
        "ECB Data Portal",
    ...
    SourceSpec(
        ...
        "PatentsView X-Api-Key",
        ("https://search.patentsview.org/api/v1/patent/",),
        "weekly",
        "provider key quota; bounded 25 latest records per weekly poll",
        env_vars=("PATENTSVIEW_API_KEY",),
        status="adapter_live_requires_key",
        notes="Search API replaces the retired endpoint. Existing provider key required.",
        operator_action="configure_existing_patentsview_key_or_wait_for_provider_issuance",
    ),
    SourceSpec(
        "github",
        "GitHub API",
