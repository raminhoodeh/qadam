...
- Current local blocker as of 2026-05-21: no Docker-compatible CLI is available on the Mac, so the bootstrap exits explicitly with `postgres_timescale_runtime_status=missing` instead of falsely claiming durable ingestion is live.
...
- Postgres/Timescale durable ingestion has contract checks that pass as `ready_waiting_for_local_service` when local Postgres is offline and fail only when live replay is explicitly required. `scripts/check_postgres_timescale_ingestion.py` stays non-strict by default and can be tightened with `--require-live`; `scripts/check_postgres_timescale_replay.py` now stays non-strict by default and can be tightened with `--require-full-source-coverage` once Docker/OrbStack/Podman/Colima is running. The dedicated bootstrap is now implemented; the remaining action is installing/opening a compatible runtime and rerunning it.
