...
- Dashboard Plan D6 Paper Account Mirror exists locally: `orchestrator/paper_account.py` stores a read-only £1000 paper-account snapshot, `scripts/check_paper_account.py` validates no live capital and no write authority, and the static dashboard renders balance, P&L, drawdown, open positions, closed trades, postmortem counts, and 0/100 maturity progress from the public-safe status contract.
...
- As of 2026-05-18, the local ignored secret file has credentials for NASA FIRMS, Alpaca paper, ACLED, FRED, Q-CTRL, Telegram bot token/username, Gemini/Google model keys, and LM Studio settings. Kalshi, Telegram default chat ID, UnusualWhales, BLS, UN Comtrade, Reddit, X, AIS/Wingbits/logistics providers, SEC user agent, IBM Quantum, and AWS Braket remain pending or deferred.
- ACLED now has access-token and refresh-token material stored locally, but automatic token renewal still needs to be implemented before ACLED counts as durable live infrastructure.
- Telegram bot credentials are local, but member delivery remains dry-run and send-disabled until `TELEGRAM_DEFAULT_CHAT_ID` and explicit send-test approval exist.
