...
- Running-now summary, health by domain, freshness, blockers, defects, repair
  queue, activity, automation state, and public-safe diagnostics.
...
- Use the lifecycle component as a stage-health view.
- Add a stage health matrix below it with one row per stage: status, item count,
  freshness, blocker, and owning destination.
