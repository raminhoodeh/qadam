...
7. OddsPipe is the selected Stage 0 read-only coverage route for normalized Kalshi/Polymarket market data while direct Kalshi signup remains region/account gated. It fills the existing Kalshi/Polymarket prediction-market monitoring slot; it is not source 36 and it has no venue execution authority.
8. Reddit Narrative Proxy is the selected first-release no-key bridge for Reddit-like retail attention while Reddit OAuth approval is pending. It uses ApeWisdom-style aggregate data and fills the existing Reddit social/narrative slot as a read-only variant; it is not source 36, not raw Reddit ingestion, and not execution authority.
...
- Generic Phase 1 read-only adapters: ACLED, UCDP, Conflict Tracker, STOCK Act via the Apify Capitol Trades Scraper, Polymarket, Kalshi/OddsPipe, Hyperliquid, Alpaca, Bookmap local bridge, AIS Maritime, ArcGIS/USACE, Space-Track/CelesTrak, GPS jamming, IODA internet outages, Aviationstack, BLS, BIS, ECB, USGS, UN Comtrade, SEC EDGAR, patents, Reddit, X, Telegram.
...
- Credential-bound read-only adapters now exist for Reddit, Kalshi/OddsPipe, and Capitol Trades/STOCK Act. The Kalshi slot activates through either direct Kalshi credentials or `ODDSPIPE_API_KEY`; the OddsPipe route is the preferred first-release read-only coverage path while direct Kalshi remains deferred.
...
- Missing or deferred from this credential batch: Reddit OAuth remains an optional future upgrade, but the first-release social narrative gap is filled by the no-key Reddit Narrative Proxy using ApeWisdom-style aggregate attention data. Direct Kalshi account eligibility remains deferred, but the existing prediction-market coverage path is satisfied through OddsPipe. Capitol Trades/STOCK Act is now routed through the Apify Capitol Trades Scraper with `CAPITOL_TRADES_API_KEY`; `CAPITOL_TRADES_API_URL` is only an override if the actor path changes. UnusualWhales is no longer a missing-key blocker.
- Configured in the local ignored secret file: NASA FIRMS, Alpaca paper, ACLED email/password/access token/refresh token, FRED, Q-CTRL, OddsPipe, Telegram bot token/username/private target/group target, Gemini/Google model keys, and LM Studio settings.
...
| Kalshi / OddsPipe | `ODDSPIPE_API_KEY`; later optional `KALSHI_API_KEY`, `KALSHI_API_SECRET`, `KALSHI_API_BASE_URL` | First-release read-only prediction-market monitoring through OddsPipe-normalized Kalshi/Polymarket markets, OHLCV context, and cross-platform spreads. Direct Kalshi remains deferred until account/region eligibility changes. |
...
| Reddit Narrative Proxy / Reddit OAuth | none for `REDDIT_NARRATIVE_PROXY=apewisdom`; later optional `REDDIT_CLIENT_ID`, `REDDIT_CLIENT_SECRET`, `REDDIT_USER_AGENT` | First-release no-key retail/forum attention through ApeWisdom aggregate data. Reddit OAuth remains a later upgrade for raw post/comment access if approved. |
...
| 7 | Aviationstack Flight Data | Physical | 2 | `AVIATIONSTACK_API_KEY` | `https://api.aviationstack.com/v1/flights` | Flight status, unusual routing, cargo, airline, airport, and aviation movement context. |
...
| 21 | Kalshi / OddsPipe | Market | 1 | `ODDSPIPE_API_KEY`; later optional `KALSHI_API_KEY`, `KALSHI_API_SECRET`, `KALSHI_API_BASE_URL` | `https://oddspipe.com/v1/spreads`; `https://oddspipe.com/v1/markets`; later `https://trading-api.kalshi.com/trade-api/v2/markets` | Read-only normalized Kalshi/Polymarket market monitoring and spread context. Direct Kalshi remains a deferred future venue path. |
...
| 31 | Reddit Narrative Proxy / Reddit API | Social | 3 | none for ApeWisdom proxy; later optional `REDDIT_CLIENT_ID`, `REDDIT_CLIENT_SECRET`, `REDDIT_USER_AGENT` | `https://apewisdom.io/api/v1.0/filter/all-stocks/page/{page}`, `https://apewisdom.io/api/v1.0/filter/all-crypto/page/{page}`, `https://apewisdom.io/api/v1.0/filter/4chan/page/{page}`; later `https://oauth.reddit.com/r/{subreddit}/new` | First-release retail attention, narrative saturation, and crowding checks via aggregate data. OAuth/raw Reddit remains optional and cannot originate trades by itself. |
...
| 33 | Capitol Trades / STOCK Act Filings | Social | 3 | `CAPITOL_TRADES_API_KEY`; optional `CAPITOL_TRADES_API_URL`, `CAPITOL_TRADES_APIFY_ACTOR_ID` | `https://api.apify.com/v2/actors/saswave~capitol-trades-scraper/run-sync-get-dataset-items` | Credential-bound politician-trading source via Apify; must be cross-validated with price action and SEC context. |
...
| Prediction markets | OddsPipe / Kalshi | `ODDSPIPE_API_KEY`; later optional `KALSHI_API_KEY`, `KALSHI_API_SECRET` | OddsPipe is read-only normalized Kalshi/Polymarket coverage. Direct Kalshi is deferred and cannot execute until a separate venue authority review exists. |
...
| Aviationstack | `AVIATIONSTACK_API_KEY` | Canonical v1 aviation provider for read-only flight and route context. |
...
BOOKMAP_LOCAL_BRIDGE_LIVE_PROBE_ENABLED=false
BOOKMAP_LOCAL_BRIDGE_TIMEOUT_SECONDS=3
...
SEC_USER_AGENT=
# STOCK Act v1 uses the Apify Capitol Trades Scraper path once supplied.
CAPITOL_TRADES_API_KEY=
CAPITOL_TRADES_APIFY_ACTOR_ID=saswave~capitol-trades-scraper
...
- STOCK Act filings are no longer provider-unselected for v1. Qadam now tracks congressional trade data through the Apify Capitol Trades Scraper as the read-only source. Activation requires `CAPITOL_TRADES_API_KEY`; `CAPITOL_TRADES_API_URL` and `CAPITOL_TRADES_APIFY_ACTOR_ID` are optional overrides. Output is evidence only and cannot approve risk or orders.
