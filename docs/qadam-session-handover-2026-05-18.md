...
- Phase 2 shadow cycle now exists as `scripts/run_phase2_shadow_cycle.py --live-sources --live-local-llm`. A 2026-05-19 live run fed FRED, RSS, Polymarket, Alpaca, and Telegram observations into the Research Analyst queue, ran local Gemma in shadow mode, and queued a Strategy Lead shadow handoff. It does not create signals with execution authority, risk approvals, or orders.
