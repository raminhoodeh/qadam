...
- Deploy via `landing-page-repo/scripts/deploy-vercel-production.sh` with the CC0 rollback ready; the asset version query has advanced through the Mission Control walkthrough overhaul to `20260619-trading-strategies-akber` after the dashboard copy runthrough removed repetitive founder-view language and the redundant visible Dashboard View card.
