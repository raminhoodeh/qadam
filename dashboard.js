...
const DASHBOARD_LEGACY_HASH_TARGETS = {
    ...
    "review-sequence": { viewId: "overview", targetId: "mission-control" },
    ...
    worldview: { viewId: "reasoning", targetId: "cognition" },
...
function setDashboardDebugControls(enabled) {
    ...
    document.querySelectorAll("[data-dashboard-debug-toggle]").forEach((button) => {
        ...
        button.setAttribute("aria-label", enabled ? "Hide advanced sections" : "Show advanced sections");
        button.classList.toggle("active", enabled);
        button.textContent = enabled ? "Hide advanced" : "Advanced";
    });
...
function buildOverviewModel(status = {}, source = {}, sharedOperations = null, sharedModels = {}) {
    ...
    const tradingStrategies = missionStrategyFamilies.length
        ? missionStrategyFamilies.map((strategy) => ({
            ...
            value: strategy.qualified_setup ? "Qualified paper setup" : "Waiting",
...
function stage7SourceGroups(status = {}, sources = {}) {
    const sourceLedger = asArray(
        status.mission_control?.data_sources?.ledger
            || status.data_sources?.ledger
            || status.source_registry?.ledger
            || status.watching
    );
...
function stage7MarketSleeves(edge = {}, strategy = {}, context = {}) {
    ...
    const sleeves = asArray(edge.sleeves).length
        ? asArray(edge.sleeves)
        : Object.entries(fallbackSymbols).map(([label, symbols]) => ({
            key: label.toLowerCase().replaceAll(" ", "_"),
            label,
            watched_instruments: symbols.map((symbol) => ({ symbol, label: symbol, paper_route: "watchlist" })),
            pattern_question: "Waiting for the source-price scan to export a pattern question.",
            strategy_use: "Market sleeve under Qadam review.",
            status: "pending"
        }));
...
function stage7LearningLoopModel(status = {}, edge = {}) {
    ...
    const memory = status.edge_memory_ledger || {};
    const lifecycle = status.hypothesis_lifecycle || {};
    const updates = status.strategy_update_record || {};
    const weights = status.strategy_weight_updates || {};
    ...
    return {
        ...
        paper_forward_status: daily.status || "not exported",
        strategy_feedback_status: updates.status || weights.status || "not exported",
        quantum_meta_review_status: meta.status || edge.quantum_pattern_review?.status || "not exported",
        candidate_pattern_count: modelNumber(daily.candidate_pattern_count || edge.pattern_ledger?.candidate_pattern_count, 0),
        validated_edge_count: modelNumber(daily.validated_edge_count || edge.pattern_ledger?.validated_edge_count, 0),
        hypothesis_thread_count: modelNumber(lifecycle.hypothesis_lifecycle_record_count || lifecycle.thread_count || lifecycle.source_hypothesis_count, 0),
        strategy_update_count: modelNumber(updates.strategy_update_count || updates.update_count || weights.strategy_weight_update_proposal_count, 0),
        ...
        cards: [
            ...
            { label: "Hypothesis lifecycle", state: lifecycle.status || "not exported", summary: "Tracks whether hypotheses are observing, ready for review, refuted, or retained." },
            { label: "Strategy feedback model", state: updates.status || weights.status || "not exported", summary: "Compares hypotheses, paper outcomes, replay evidence, and proposed strategy weights." },
            { label: "Quantum meta-review", state: meta.status || edge.quantum_pattern_review?.status || "not exported", summary: meta.purpose || "Checks non-linear learning coherence before any proposal can be promoted." },
            { label: "Promotion gates", state: promotion.status || "not exported", summary: "Keeps strategy changes human-governed and prevents silent mutation." }
        ]
    };
}
...
function missionControlTeamRoleLabel(role = {}) {
    ...
    const labels = {
        safety_policy: "Risk Desk",
        risk_agent: "Risk Desk",
        signal_integrity_gate: "Risk Desk",
        paper_demo_state: "Paper Trading Desk",
        broker_reconciliation: "Paper Trading Desk",
        execution_adapter_status: "Paper Trading Desk"
    };
...
function buildStage7VisibilityModel(status = {}, models = {}) {
    ...
    const reasoningCounts = reasoning.counts || {};
    const missionStrategy = status.mission_control?.strategy || {};
    const paper = stage7PortfolioModel(status, contract, performance);
    ...
    const strategyFamilies = asArray(missionStrategy.strategy_families).length
        ...
        : asArray(overview.trading_strategies);
    const strategyModel = {
        ...missionStrategy,
        strategy_families: strategyFamilies,
        families: strategyFamilies,
        strategy_family_count: modelNumber(missionStrategy.strategy_family_count || overview.strategy_family_count, strategyFamilies.length),
        qualified_strategy_family_count: modelNumber(missionStrategy.qualified_strategy_family_count || overview.qualified_strategy_family_count, strategyFamilies.filter((strategy) => strategy.qualified_setup).length),
        akber_lens: missionStrategy.akber_lens || {
            stages: ["context", "catalyst", "confirmation", "risk", "execution", "postmortem learning"],
    ...
    return {
        ...
        stage: "Mission Control Walkthrough",
        status: "mission_control_walkthrough_ready",
        legacy_status: "stage7_visibility_ready",
        level_1_section_count: sections.length,
        level_1_sections: sections,
        fund_brief: {
            title: "Today's Fund Brief",
            body: paperFundBrief,
            lifecycle: [
                "Watching",
                "Pattern detected",
        ...
        watched_markets_universe: {
            ...
            watched_instrument_count: modelNumber(edge.watched_instrument_count, marketSleeves.reduce((total, sleeve) => total + asArray(sleeve.watched_instruments).length, 0)),
            sleeves: marketSleeves,
            price_watch_status: edge.market_price_watch?.status || "listed",
            boundary: edge.market_price_watch?.live_price_boundary || "Watched markets are research and paper-route context, not broker truth."
        },
        strategy_playbook: {
        ...
        operating_map: {
            ...
            edges: [
                ...
                { from: "strategies", to: "team", state: "solid", label: "research and quant review" },
                { from: "team", to: "hypotheses", state: "dashed", label: "candidate patterns and hypotheses" },
                { from: "hypotheses", to: "paper", state: "locked", label: "risk and PaperOps boundary" },
                { from: "paper", to: "learning", state: "solid", label: "outcomes and replay" }
            ],
            sentence: "World data comes in, Qadam maps it to watched markets, filters it through strategy and the investment team, forms hypotheses, paper trades only through guarded routes, then learns from outcomes."
        },
        system_status: {
            rows: [
                stage7StatusRecord("fund", "Paper fund", paper.connection_status || paper.mirror_freshness || "online", `${formatMoney(paper.current_balance_gbp || paper.balance_gbp)} current value; ${paper.open_position_count || 0} open positions.`),
                stage7StatusRecord("sources", "Sources", sources.quorum?.status || sources.tone, `${sourceCounts.online || 0}/${sourceCounts.total || 0} sources online.`),
                stage7StatusRecord("reasoning", "Reasoning", strategyReview.status || localReview.status || reasoning.tone, `${reasoningCounts.hypotheses || 0} hypotheses and ${reasoningCounts.evidence_packets || 0} evidence packets.`),
        ...
        data_sources: {
            ...
            signal_review_eligible_count: sourceCounts.signal_review_eligible || 0,
            groups: sourceGroups,
            influencing_groups: sourceGroups.filter((group) => modelNumber(group.signal_review_eligible_count, 0)).slice(0, 4)
        },
        trading_strategies: {
            mandate: "Grow the GBP 100,000 paper portfolio toward GBP 200,000 over 60 days.",
        ...
        paper_portfolio_capacity: {
            ...
            baseline_gbp: paper.starting_balance_gbp || 100000,
            total_gbp: paper.starting_balance_gbp || 100000,
            target_gbp: paper.target_gbp || 200000,
            equity_gbp: paper.current_balance_gbp || paper.balance_gbp,
            idle_gbp: paper.cash_gbp || overview.paper_capacity?.cash_gbp || 0,
            open_risk_gbp: overview.paper_capacity?.deployed_gbp || 0,
            realized_pnl_gbp: paper.realized_pnl_gbp || 0,
            unrealized_pnl_gbp: paper.unrealized_pnl_gbp || 0,
            equity_curve: asArray(paper.equity_curve),
            history_sparse: asArray(paper.equity_curve).length < 3
        },
        authority: {
            dashboard_write_authority: false,
            broker_write_allowed: false,
...
function normalizeContractSource(source = {}) {
    return {
        key: source.source_key || source.key || source.source_name || "source",
        label: source.source_name || source.label || source.source_key || "Source",
        pipeline: source.pipeline || "uncategorized",
        status: source.status || "pending",
        credential_status: source.credential_status || "unknown",
        readiness: source.readiness || "status exported",
        trust_score: source.trust_score,
        ...
        summary: [
            source.readiness,
            source.usable_for_research_context ? "research usable" : "observation only",
            source.eligible_for_signal_review ? "signal-review eligible" : "not signal-review eligible"
        ].filter(Boolean).join("; "),
        boundary: source.boundary || "Observation input only."
    };
}

function buildContractSourcePipelines(ledger = []) {
    const groups = new Map();
    asArray(ledger).forEach((source) => {
        const pipelineKey = source.pipeline || "uncategorized";
        if (!groups.has(pipelineKey)) {
            groups.set(pipelineKey, {
                pipeline: pipelineKey,
                label: OPERATIONS_PIPELINE_LABELS[pipelineKey] || pipelineKey,
                sources: [],
                source_count: 0,
                online_count: 0,
                degraded_count: 0,
                missing_credential_count: 0,
                research_usable_count: 0,
                signal_review_eligible_count: 0,
                order_authority_count: 0
            });
        }
        const group = groups.get(pipelineKey);
        group.sources.push(source);
        group.source_count += 1;
        if (source.status === "online") group.online_count += 1;
        if (source.status === "degraded") group.degraded_count += 1;
        if (source.credential_status === "missing") group.missing_credential_count += 1;
        if (source.usable_for_research_context) group.research_usable_count += 1;
        if (source.eligible_for_signal_review) group.signal_review_eligible_count += 1;
        if (source.can_authorize_orders) group.order_authority_count += 1;
    });
    return Array.from(groups.values()).map((group) => ({
        ...group,
        status: group.degraded_count || group.missing_credential_count
            ? "degraded"
            : (group.online_count ? "online" : "pending")
    }));
}
...
function buildFounderContractModel(status = {}, source = {}, sharedModels = {}) {
    ...
    const tradingPhilosophy = mission.trading_philosophy || {};
    const trades = mission.trades || {};
    const tradeIntent = mission.trade_intent || {};
    const thinking = mission.thinking || {};
    const safety = mission.safety || {};
    const durableSpine = mission.durable_spine || status.durable_ingestion || {};
    const team = asArray(mission.team).map((member) => ({
        key: member.key || member.label || "team_member",
        label: member.label || "Qadam team node",
        subtitle: member.owner || member.authority || "Qadam",
        status: member.status || "pending",
        summary: member.one_line || member.current_process || "Status exported from Mission Control.",
        current_process: member.current_process || member.one_line || "No current process exported.",
        authority: member.authority || "status only"
    }));
    const ledger = asArray(dataSources.ledger).map(normalizeContractSource);
    const sourcePipelines = buildContractSourcePipelines(ledger);
    const sourceCounts = {
        total: modelNumber(dataSources.total_count, ledger.length),
        online: modelNumber(dataSources.online_count ?? dataSources.ok, ledger.filter((item) => item.status === "online").length),
        degraded: modelNumber(dataSources.degraded_count ?? dataSources.degraded, ledger.filter((item) => item.status === "degraded").length),
        missing_credentials: modelNumber(dataSources.missing_credential_count ?? dataSources.missing_credentials, ledger.filter((item) => item.credential_status === "missing").length),
        local_only: modelNumber(dataSources.local_only_count, ledger.filter((item) => item.status === "local_only").length),
        signal_review_eligible: ledger.filter((item) => item.eligible_for_signal_review).length,
        research_usable: ledger.filter((item) => item.usable_for_research_context).length,
    ...
    return {
        ...
        safety: {
            ...
            live_capital_enabled: Boolean(safety.live_capital_enabled),
            broker_write_route: safety.broker_write_route || "closed",
            forbidden_action_count: modelNumber(safety.forbidden_action_count, 0),
            hard_blocks: asArray(safety.hard_blocks),
            boundary: safety.boundary || "Mission Control is read-only."
        },
        durable_spine: {
            status: durableSpine.status || durableSpine.contract_status || "pending",
            replay_status: durableSpine.replay_status || durableSpine.contract_status || durableSpine.status || "pending",
            contract_status: durableSpine.contract_status || durableSpine.status || "pending",
            replayed_source_count: modelNumber(durableSpine.replayed_source_count, 0),
            expected_source_count: modelNumber(durableSpine.expected_source_count, sourceCounts?.total || 0),
            observation_count: modelNumber(durableSpine.observation_count, 0),
            boundary: durableSpine.boundary || "Durable replay is observation storage only."
        },
        team,
        sources: {
            ...sourceCounts,
            status: dataSources.phase1_data_spine_operational_status || dataSources.phase1_data_spine_status || dataSources.quorum?.status || "pending",
            quorum: dataSources.quorum || {},
            ledger,
            pipelines: sourcePipelines,
            durable_replay_status: dataSources.durable_replay_status || "unknown",
            durable_replayed_source_count: modelNumber(dataSources.durable_replayed_source_count, 0),
            durable_expected_source_count: modelNumber(dataSources.durable_expected_source_count, 0),
            boundary: dataSources.boundary || "Sources are observation inputs only."
        },
        strategy: {
        ...
        portfolio: {
            account_scope: portfolio.account_scope || "paper account",
        ...
        trades: {
            state: tradeIntent.state || "pending",
            summary: tradeIntent.summary || "No trade-intent summary exported.",
            lifecycle_counts: {
                observed: modelNumber(lifecycleCounts.observed, modelNumber(tradeIntent.observed_signal_count, 0)),
                candidate: modelNumber(lifecycleCounts.candidate, modelNumber(tradeIntent.candidate_count, 0)),
                blocked: modelNumber(lifecycleCounts.blocked, modelNumber(tradeIntent.blocked_count, 0)),
                open: modelNumber(lifecycleCounts.open, modelNumber(portfolio.open_position_count, 0)),
                closed: modelNumber(lifecycleCounts.closed, modelNumber(portfolio.closed_trade_count, 0)),
                paper_order_submitted: modelNumber(lifecycleCounts.paper_order_submitted, modelNumber(tradeIntent.paper_order_submitted_count, 0)),
                postmortem_due: modelNumber(lifecycleCounts.postmortem_due, modelNumber(portfolio.postmortem_due_count, 0))
            },
            board: asArray(trades.board),
            open: asArray(trades.open),
            postmortems_due: asArray(trades.postmortems_due),
            top_candidates: asArray(tradeIntent.top_candidates),
            blocked_trades: asArray(tradeIntent.blocked_trades),
            boundary: trades.boundary || tradeIntent.boundary || "Trade intent is non-executing."
        },
        thinking: {
            status: thinking.status || thinking.phase2_status || "pending",
            phase2_mode: thinking.phase2_mode || "not exported",
            research_goal_active_count: modelNumber(thinking.research_goal_active_count, 0),
            hypothesis_count: modelNumber(thinking.hypothesis_count, asArray(thinking.hypotheses).length),
            evidence_packet_count: modelNumber(thinking.evidence_packet_count, 0),
            strategy_packet_count: modelNumber(thinking.strategy_packet_count, 0),
            local_assessment_count: modelNumber(thinking.local_assessment_count, 0),
            signal_integrity_status: thinking.signal_integrity_status || "pending",
            research_goals: asArray(thinking.research_goals),
            hypotheses: asArray(thinking.hypotheses),
            missing_corroboration: asArray(thinking.missing_corroboration),
            worldview_prior: thinking.worldview_prior || {},
            boundary: thinking.boundary || "Reasoning is research-only."
        },
        diagnostics_linked_models: {
            sources_model: sharedModels.sources_model?.id || "diagnostics_sources",
            trades_model: sharedModels.trades_model?.id || "diagnostics_trades",
            reasoning_model: sharedModels.reasoning_model?.id || "diagnostics_reasoning",
            performance_model: sharedModels.performance_model?.id || "diagnostics_performance",
            operations_model: sharedModels.operations_model?.id || "diagnostics_operations"
        }
    };
}
...
function renderOperatingSummary(status, source) {
    ...
    target.innerHTML = [
        ...
        renderPriorityCard(
            ...
            "Paper orders require source, risk, and broker-route gates",
            paperOrders ? "pending" : "online"
        ),
        renderPriorityCard(
            "Safety boundary",
            liveCapital ? "Authority review" : "Live capital off",
            `${forbiddenActions.length} safety stops · broker writes ${brokerWriteBlocked ? "stopped" : "not recorded"} · paper-only dashboard authority`,
            "No dashboard control can place or approve an order",
...
function overviewNodeGuide(node = {}, role = "", label = "") {
    ...
    const boundary = overviewPlainText(node.authority || expanded.current_status, "Dashboard status.");
    return {
...
function renderContractMissionSnapshot(contract = {}) {
    ...
    return `
        <div class="overview-section-head">
            ...
            <strong>${htmlText(contract.status || "Current operating state")}</strong>
        </div>
        <div class="overview-mission-metrics">
            ${renderOverviewMissionMetric({ label: "Sources", value: `${sources.online || 0}/${sources.total || 0}` })}
            ${renderOverviewMissionMetric({ label: "Replay", value: `${durable.replayed_source_count || 0}/${durable.expected_source_count || sources.total || 0}` })}
            ${renderOverviewMissionMetric({ label: "Candidates", value: trades.lifecycle_counts?.candidate || 0 })}
            ${renderOverviewMissionMetric({ label: "Hypotheses", value: thinking.hypothesis_count || 0 })}
            ${renderOverviewMissionMetric({ label: "Paper account", value: formatMoney(portfolio.balance_gbp) })}
...
function portfolioTradeRecordReason(record = {}, type = "order") {
    const explicitReason = record.why || record.reason || record.summary || record.evidence_summary || record.catalyst || record.close_reason;
    if (explicitReason) return overviewPlainText(explicitReason);
    const status = String(record.status || record.order_status || "").toLowerCase();
    const direction = String(record.direction || record.side || "").toLowerCase();
    if (type === "position") {
        return "Open paper position mirrored from Alpaca and tracked against current account value.";
    }
    if (direction === "sell") {
    ...
    return "Mirrored from the Alpaca paper account; no separate strategy note was exported for this row.";
}

function portfolioTradeTime(record = {}) {
    return record.filled_at || record.submitted_at || record.created_at || record.opened_at || record.observed_at || record.updated_at;
}

function portfolioTradePrice(record = {}) {
    const value = firstPresent(record.filled_avg_price, record.current_price, record.entry_price, record.limit_price, record.stop_price);
    if (value === null || value === undefined || value === "") return "market";
    const number = Number(value);
    return Number.isFinite(number) ? number.toLocaleString("en-GB", { maximumFractionDigits: 2 }) : dashboardText(value);
}

function renderPortfolioTradeTimelineRow(record = {}, type = "order") {
    const symbol = record.symbol || record.instrument || record.asset || record.ticker || "Instrument";
    const direction = dashboardText(record.direction || record.side || (type === "position" ? "long" : "order"));
    const status = dashboardText(record.status || record.order_status || (type === "position" ? "open position" : "recorded"));
    const quantity = firstPresent(record.quantity, record.qty, record.filled_quantity, record.shares, record.position_qty, "n/a");
    return `
        <li class="portfolio-trade-row ${statusClass(status)}">
            <div>
                <strong>${htmlText(symbol)}</strong>
                <span>${htmlText(direction)} · ${htmlText(status)} · ${htmlText(formatTime(portfolioTradeTime(record)))}</span>
            </div>
            <dl>
                <div><dt>Qty</dt><dd>${htmlText(quantity)}</dd></div>
                <div><dt>Price</dt><dd>${htmlText(portfolioTradePrice(record))}</dd></div>
                <div><dt>Why</dt><dd>${htmlText(portfolioTradeRecordReason(record, type))}</dd></div>
            </dl>
        </li>
    `;
}

function renderPortfolioTradeTimeline(portfolio = {}) {
    const orders = asArray(portfolio.orders);
    const positions = asArray(portfolio.open_positions).map((position) => ({
        ...position,
        direction: position.direction || position.side || "long",
        status: position.status || "open position"
    }));
    const buyRecords = [
        ...orders.filter((order) => /buy|long/i.test(String(order.direction || order.side || ""))),
        ...positions.filter((position) => !/short|sell/i.test(String(position.direction || position.side || "")))
    ].slice(0, 80);
    const sellRecords = orders
        .filter((order) => /sell|close|short/i.test(String(order.direction || order.side || "")))
        .slice(0, 80);
    const renderEmpty = (message) => `<li class="portfolio-trade-row empty"><span>${htmlText(message)}</span></li>`;
    return `
        <details class="portfolio-trade-timeline" data-portfolio-trade-timeline>
            <summary>
                <span>Trade timeline</span>
                <strong>${buyRecords.length} bought or held · ${sellRecords.length} sold or closed</strong>
                <em>Expand trade history</em>
            </summary>
            <div class="portfolio-trade-timeline-grid">
                <section>
                    <h4>Bought and held</h4>
                    <ul class="portfolio-trade-list">
                        ${buyRecords.length ? buyRecords.map((record) => renderPortfolioTradeTimelineRow(record, positions.includes(record) ? "position" : "order")).join("") : renderEmpty("No buy or open-position rows exported in this snapshot.")}
                    </ul>
                </section>
                <section>
                    <h4>Sold and closed</h4>
                    <ul class="portfolio-trade-list">
                        ${sellRecords.length ? sellRecords.map((record) => renderPortfolioTradeTimelineRow(record, "order")).join("") : renderEmpty("No sell or close rows exported in this snapshot.")}
                    </ul>
                </section>
            </div>
        </details>
...
function renderContractPortfolioHero(portfolio = {}) {
    const block = renderContractPortfolioBlock(portfolio);
    return `
        <div class="portfolio-hero-shell" data-portfolio-hero-shell>
            ${block}
...
function renderStrategySummaryStrip(strategy = {}, strategyFamilies = []) {
    ...
    return `
        <div class="strategy-summary-strip">
            ${rows.map((row) => `
                <article class="${statusClass(row.tone)}">
                    <span>${htmlText(row.label)}</span>
                    <strong>${htmlText(row.value)}</strong>
                    <p>${htmlText(row.detail)}</p>
                </article>
            `).join("")}
        </div>
    `;
...
function renderContractTeamMap(team = []) {
    const nodes = asArray(team);
    if (!nodes.length) return `<span>No Mission Control team nodes are visible yet.</span>`;
    const nodeHtml = nodes.map((member, index) => renderOverviewMiniNode({
        key: member.key,
        label: member.label,
        role: member.subtitle,
        status: member.status,
        health: member.status,
        purpose: member.summary,
        ...
        <div class="control-plane-shell" data-cc5-contract-source="mission_control">
            ...
            <div class="control-plane-grid">
                ...
                <aside class="control-plane-oversight">
                    ...
                    <p>Live data moves through the Python COO, Local LLM, Strategy Lead, Head of Quant, risk gate, paper lifecycle, and learning loop.</p>
                    <p>The dashboard shows status only. It does not approve trades, write to brokers, fund accounts, or enable live capital.</p>
                </aside>
            </div>
            ...
            <div class="overview-boundary-rail control-plane-boundary">
                ...
                <p>Trade ideas stay candidates until source, risk, and PaperOps checks create a valid paper-order route.</p>
            </div>
        </div>
    `;
}

function renderContractSourceRow(source = {}) {
    const trust = source.trust_score === undefined || source.trust_score === null ? "n/a" : Number(source.trust_score).toFixed(2);
    return `
        <li class="source-row overview-source-row">
            <div class="source-main">
                ${renderStatusPill(source.status)}
            ...
            <div class="source-meta">
                ${renderInlineBadge(source.credential_status, source.credential_status === "missing" ? "degraded" : "online")}
                ${renderInlineBadge(source.usable_for_research_context ? "research usable" : "observation only", source.usable_for_research_context ? "online" : "pending")}
                ${renderInlineBadge(source.eligible_for_signal_review ? "signal-review eligible" : "not signal-review eligible", source.eligible_for_signal_review ? "online" : "pending")}
                ${renderInlineBadge(source.can_authorize_orders ? "order authority" : "no order authority", source.can_authorize_orders ? "blocked" : "online")}
                ${renderInlineBadge(`trust ${trust}`, trust === "n/a" ? "pending" : "online")}
                ${renderInlineBadge(source.latency_ms ? `${Math.round(Number(source.latency_ms))}ms` : formatTime(source.heartbeat), source.status)}
            </div>
            <p>${htmlText(source.summary || source.degraded_reason || source.boundary)}</p>
        </li>
    `;
}

function renderContractSourcePipeline(pipeline = {}) {
    const sources = asArray(pipeline.sources);
    return `
        <details class="overview-ledger-group">
            <summary>
                <strong>${htmlText(pipeline.label || pipeline.pipeline, "Source pipeline")}</strong>
                <span>${htmlText(pipeline.online_count, "0")}/${htmlText(pipeline.source_count, sources.length)} online · ${htmlText(pipeline.research_usable_count, "0")} research usable · ${htmlText(pipeline.signal_review_eligible_count, "0")} signal-review eligible · ${htmlText(pipeline.missing_credential_count, "0")} missing credentials</span>
            </summary>
            <ul class="source-table">${sources.map(renderContractSourceRow).join("")}</ul>
        </details>
    `;
}

function renderContractStrategyBlock(contract = {}) {
    const strategy = contract.strategy || {};
    const activeLens = strategy.active_lens || {};
    const akber = strategy.akber_lens || {};
    ...
    return `
        <details class="overview-expandable-ledger strategy-universe-ledger" data-overview-strategy-ledger data-cc5-contract-source="mission_control" open>
            <summary>
...
function renderContractStrategyNarrative(contract = {}) {
    const strategy = contract.strategy || {};
    const sources = contract.sources || {};
    const trades = contract.trades || {};
    const thinking = contract.thinking || {};
    ...
    return `
        <div class="strategy-narrative-shell ${statusClass(strategy.posture || activeLens.status || "online")}" data-cc5-contract-source="mission_control">
            <article class="strategy-narrative-lead">
                ...
                <p>${htmlText(sourceSentence)} ${htmlText(thinkingSentence)} ${htmlText(tradeSentence)}</p>
            </article>
    ...
    return `
        <div class="paper-trade-state-shell ${statusClass(mirrorTone)}" data-cc5-contract-source="mission_control">
            ...
            <div class="paper-trade-state-grid">
                <section class="paper-account-state-card">
                    <div class="overview-capacity-summary">
                        ${renderMetric("Balance", formatMoney(portfolio.balance_gbp))}
                        ${renderMetric("Current value", formatMoney(portfolio.current_balance_gbp || portfolio.balance_gbp))}
                    ...
                    <p class="mini">${htmlText(dashboardText(portfolio.portfolio_value_source, "Paper values come from the Alpaca paper-account mirror."))}</p>
                </section>
...
function renderContractSourceSummary(sources = {}) {
    ...
    return `
        <div class="overview-source-summary-card ${statusClass(sourceTone)}" data-cc5-contract-source="mission_control">
            ...
            <details class="overview-expandable-ledger source-universe-ledger">
                <summary>
                    <span>Source list</span>
                    <strong>${htmlText(sources.total || 0)} sources across ${htmlText(pipelines.length || 0)} visible pipelines</strong>
                    <em>Expand source health</em>
                </summary>
                <div class="overview-ledger-body">
...
function renderMissionLifecycleRibbon(items = []) {
    return `
        <ol class="mission-flow-lifecycle" aria-label="Qadam decision lifecycle">
            ${asArray(items).map((item, index) => `
                <li>
                    <span>${String(index + 1).padStart(2, "0")}</span>
                    <strong>${htmlText(item)}</strong>
                </li>
            `).join("")}
        </ol>
    `;
...
function renderMissionStrategies(stage7 = {}) {
    ...
    const akberStages = asArray(playbook.akber_lens?.stages);
    return `
...
function renderMissionTeam(stage7 = {}) {
    ...
    return `
        <section class="stage7-section mission-flow-section mission-team" data-stage7-section="hedge_fund_investment_team">
    ...
    return `
        <section class="stage7-section mission-flow-section mission-hypotheses" id="hypotheses_pattern_recognition" data-stage7-section="hypotheses_pattern_recognition">
            ...
            <div class="stage7-kpi-strip mission-kpi-strip">
                ...
                ${renderMetric("Validated edges", pattern.validated_edge_count || 0)}
    ...
    return `
        <section class="stage7-section mission-flow-section mission-learning" data-stage7-section="backtesting_learning_loop">
            ...
            <div class="stage7-kpi-strip mission-kpi-strip">
                ...
                ${renderMetric("Replay", learning.replay_status || "not exported")}
                ${renderMetric("Patterns", learning.candidate_pattern_count || 0)}
                ${renderMetric("Validated edges", learning.validated_edge_count || 0)}
...
function renderStage7Visibility(viewModels = {}) {
    ...
    target.innerHTML = `
        <div class="stage7-visibility-shell mission-control-walkthrough" data-stage7-rendered data-stage7-contract="mission_control_walkthrough_v1">
            <div class="stage7-hero mission-flow-hero">
                <div>
                    <p class="label">${htmlText(stage7.fund_brief?.title, "Today's Fund Brief")}</p>
                    <h2>Qadam as a paper hedge fund</h2>
                    <p>${htmlText(stage7.fund_brief?.body, "Qadam's paper-fund brief has not loaded yet.")}</p>
                </div>
                <div class="stage7-kpi-strip compact">
                    ${renderMetric("Sections", stage7.level_1_section_count || 0)}
                    ${renderMetric("Sources", `${stage7.source_intelligence_network?.online || 0}/${stage7.source_intelligence_network?.total || 0}`)}
                    ${renderMetric("Markets", stage7.watched_markets_universe?.sleeve_count || 0)}
                    ${renderMetric("Patterns", stage7.hypotheses_pattern_recognition?.candidate_pattern_count || 0)}
                    ${renderMetric("Paper value", formatMoney(stage7.paper_fund_status?.current_value_gbp))}
                </div>
            </div>
            ${renderMissionLifecycleRibbon(stage7.fund_brief?.lifecycle)}
            ${renderMissionPaperFund(stage7)}
            ${renderMissionSourceNetwork(stage7)}
            ${renderMissionMarkets(stage7)}
            ${renderMissionStrategies(stage7)}
...
function renderOverviewFirstScreen(viewModels) {
    const contract = viewModels?.founder_contract_model;
    if (!contract) return;

    const statusRail = dashboardQuery("[data-overview-status-rail]");
    if (statusRail) {
        const source = contract.sources || {};
        const portfolio = contract.portfolio || {};
    ...
    const portfolioHeroTarget = dashboardQuery("[data-overview-portfolio-hero]");
    if (portfolioHeroTarget) {
        portfolioHeroTarget.innerHTML = renderContractPortfolioHero(contract.portfolio || {});
    }
    ...
    setText("[data-overview-lifecycle-summary]", contract.trades?.summary || "Mission Control trade lifecycle is loaded.");
    const lifecycle = dashboardQuery("[data-overview-lifecycle]");
    if (lifecycle) {
        lifecycle.innerHTML = Object.entries(contract.trades?.lifecycle_counts || {})
            .map(([label, count], index) => renderOverviewLifecycleItem({ label: label.replaceAll("_", " "), count, tone: count ? "online" : "pending" }, index))
            .join("");
    }
