...
assert(quantum.empirical_comparison_count === 0, "Current protocol placeholders counted as empirical comparisons");
...
assert(quantum.reviews.every((row) => row.returned_to === "Pattern Discovery"), "Quantum verdict does not return to Pattern Discovery");
assert(quantum.reviews.every((row) => row.hardware_used === false), "Quantum hardware use misrepresented");
assert(quantum.reviews.every((row) => row.verdict === "not_measurable"), "Current quantum verdict should remain not measurable");
