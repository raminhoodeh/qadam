...
def test_broker_mirror_history_is_not_presented_as_qadam_origin_activity():
    context = _complete_context()
    context["lifecycle"] = {
        "broker_record_count": 42,
        "origin_counts": {"mirror_only_historical_record": 42},
        "ambiguous_lifecycle_count": 0,
    }
    context["current_portfolio"] = {"position_count": 0}
    summary = build_lifecycle_dashboard_summary(
        context,
        generated_at="2026-07-12T08:00:00+00:00",
    )
    execution = summary["stage_states"]["execute_monitor"]
    assert execution["state"] == "idle"
    assert execution["item_count"] == 42
    assert "42 broker-mirror records are historical reference only" in execution["summary"]
    assert any(
        metric == {"label": "Qadam-origin lifecycle", "value": 0} for metric in execution["metrics"]
    )
