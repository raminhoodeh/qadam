...
def test_operator_dispatch_proves_ownership_during_status_refresh_race(
    tmp_path: Path, monkeypatch
) -> None:
    monkeypatch.setattr(supervisor, "runtime_dir", lambda settings=None: tmp_path)
    monkeypatch.setattr(
    ...
    _write(tmp_path / supervisor.OPERATOR_STATUS_ARTIFACT, {})
    for name in (
        supervisor.ATOMICITY_CHECK_ARTIFACT,
        supervisor.RESUME_CHECK_ARTIFACT,
    ):
    ...
    assert checks["status"] == "passed"
    assert status["scheduler_owner"] == "qadam_operator_service"
    assert status["superseded_by_operator_service"] is True
