...
def _build_gap_closure(tmp_path_factory: pytest.TempPathFactory):
    global RUNTIME
    RUNTIME = tmp_path_factory.mktemp("plbg-empty-runtime")
    # Empty history is deliberate. The builder must not borrow the operator's
    # untracked evidence or assert completion merely because code is installed.
    with pytest.MonkeyPatch.context() as patch:
        patch.setattr(closure, "ROOT", RUNTIME.parent)
        yield closure.build_all(replace(Settings.from_env(), runtime_dir=str(RUNTIME)))
...
def test_all_implementation_stages_and_evidence_certification_pass_honestly():
    ...
    assert certification["evidence_program_complete"] is False
    ...
    assert any(stage["status"] != "passed" for stage in certification["stage_checks"].values())
...
def test_current_universe_and_legacy_learning_are_reconciled_without_authority():
    ...
    assert contract["canonical_source_count"] == 0
    assert contract["canonical_watched_instrument_count"] == 0
...
def test_every_source_and_instrument_gap_has_a_typed_state():
    ...
    assert sum(matrix["source_state_counts"].values()) == 0
    assert sum(matrix["instrument_state_counts"].values()) == 0
    assert empirical["empirically_scored_source_count"] == 0
    assert empirical["empirically_tested_instrument_count"] == 0
    assert empirical[
...
def test_focus_provider_limits_remain_explicit():
    ...
    assert backtest["v4_focus_empirical_complete"] is False
    ...
    assert all(experiment["measured_hypothesis_count"] == 0 for experiment in backtest["focus_experiments"])
    assert all(
