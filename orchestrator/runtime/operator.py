...
from orchestrator.runtime.services import SERVICE_DEFINITIONS, ServiceDefinition
...
from orchestrator.contracts.timestamps import _parse_timestamp as _parse_timestamp
from orchestrator.runtime.recovery_policy import (
    classify_failure as classify_failure,
    retry_policy as retry_policy,
)
from orchestrator.runtime.scheduling import (
    _next_due_at as _next_due_at,
    _is_due as _is_due,
...
def _default_command_executor(command: tuple[str, ...], timeout_seconds: int) -> dict[str, Any]:
    ...
    return run_command(command, timeout_seconds, root=ROOT, sanitize=_sanitize_process_output)


def _result_is_evidence_hold(result: dict[str, Any]) -> bool:
    work = result.get("work_result") or {}
    hold_states = (
        "evidence_maturing", "hold", "blocked_evidence_maturing",
        "complete_for_supported_sources", "running_idle_no_eligible_hypothesis",
    )
    return (
        int(result.get("returncode") or 0) != 0
        and work.get("validation_error_count") == 0
        and work.get("status") in hold_states
    )
...
def _result_is_optional_publication_transport_hold(
    ...
    credential_status = re.search(r"http_status_(?:401|403)\b", reason)
    return (
        int(result.get("returncode") or 0) != 0
        and work.get("status") == "degraded"
        and (
            reason.startswith("transport_error:")
            or reason in {"receiver_not_configured", "public_status_receiver_not_configured"}
        )
    ...
    for name in definition.generation_artifacts:
        ...
        if not path.is_file():
            ...
            if path.suffix != ".jsonl":
                raise GenerationError(f"generation_required_artifact_missing:{name}")
            continue
                ...
                command_results.append(
                    {
                        ...
                        "work_result": result.get("work_result") or {},
...
def _service_code_state(definition: ServiceDefinition) -> list[dict[str, Any]]:
    from orchestrator.runtime.builds import reviewed_source_state
    return reviewed_source_state(ROOT)
