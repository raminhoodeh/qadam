...
def _comparison_contribution(record: dict[str, Any]) -> str:
    ...
    if verdict in {
        ...
        return "incremental"
    if verdict == "not_useful_for_this_edge":
        return "not_useful"
    if record.get("fallback_used") is True or verdict.startswith("not_measurable"):
        return "fallback"
    return "neutral"
...
def _representative_comparison(
    ...
    empirical = [record for record in comparisons if _empirical_comparison(record)]
    if not empirical:
        return {}
    priority = {
        "incremental": 0,
    ...
    return min(
        empirical,
        key=lambda record: (
            priority[_comparison_contribution(record)],
            0 if "quantum" in str(record.get("method") or "") else 1,
            -abs(safe_float(record.get("incremental_holdout_value"))),
        ),
    )
...
def _relationship_stage(
    ...
    edge_state = str(
        edge.get("edge_state") or edge.get("promotion_class") or edge.get("confidence_class") or ""
    ).lower()
...
def _relationship_record(
    ...
    *,
    ...
    instruments = sorted(
        {str(record.get("instrument")) for record in records if record.get("instrument")}
    )
    ...
    return {
        ...
        "fresh_source_ratio": round(fresh_source_count / len(unique_inputs), 6)
        if unique_inputs
        else 0.0,
        ...
        "strategy_fit": str(records[0].get("strategy_label") or family.replace("_", " ").title()),
...
def build_pattern_discovery_projection(
    *,
    ...
    highest = max(
        relationships, key=lambda row: safe_float(row.get("raw_pattern_score")), default=None
    )
    qualitative_bullets = []
    ...
    analysis_summary = (
        f"The latest scoring pass produced {len(relationships)} distinct research observations. "
    )
    if highest:
        analysis_summary += (
    ...
    return {
        ...
        "funnel": [
            ...
            {
                "key": "eligible",
                "label": "Eligible historical snapshots",
                "count": eligible_snapshots,
            },
            ...
            {
                "key": "active",
                "label": "Active live matches",
                "count": sum(row["current_live_match"]["is_active"] for row in relationships),
            },
        ],
        "tabs": [
            {
                "key": "live_observations",
                "label": "Live observations",
                "count": tab_counts["live_observations"],
            },
            {
                "key": "under_testing",
                "label": "Under testing",
                "count": tab_counts["under_testing"],
            },
            {
                "key": "validated_edges",
                "label": "Validated edges",
                "count": tab_counts["validated_edges"],
            },
            {
                "key": "rejected_or_decayed",
                "label": "Disproved or faded",
                "count": tab_counts["rejected_or_decayed"],
            },
        ],
...
def _quantum_verdict(
    ...
    contributions = [_comparison_contribution(row) for row in empirical]
    if "incremental" in contributions:
        return (
            "nonlinear_strengthened",
            "A controlled nonlinear or quantum comparison earned incremental holdout value beyond the matched classical baseline. This remains research evidence only.",
            "Integrated validation",
            "incremental",
        )
    if "not_useful" in contributions:
        return (
            "classical_preferred",
            "The quantum comparison ran, but it did not earn incremental edge credit after reliability, complexity and false-discovery controls. The classical baseline remains preferred.",
            "Classical-only result",
            "not_useful",
        )
    return (
        "inconclusive",
        "The empirical comparison did not produce a reliable controlled difference, so no incremental edge credit was granted.",
        "Under testing",
        "neutral",
    )
...
def build_quantum_review_projection(
    *,
    ...
    for family in family_ids:
        ...
        simulator_total += sum(row.get("simulation_used") is True for row in empirical)
        mode, mode_label = _quantum_execution_mode(family_protocols, family_comparisons)
        verdict, verdict_text, returned_stage, contribution = _quantum_verdict(family_comparisons)
        representative = _representative_comparison(family_comparisons)
            ...
            method_rows.append(
                {
                    ...
                    if empirical_method
                    else "waiting_for_untouched_holdout",
                    "state_label": "Comparison complete"
                    if empirical_method
                    else "Not run; waiting for untouched holdout",
                    "classical_holdout_metric": comparison.get("classical_holdout_metric"),
                    "nonlinear_or_quantum_holdout_metric": comparison.get(
                        "nonlinear_or_quantum_holdout_metric"
                    ),
                    "incremental_holdout_value": comparison.get("incremental_holdout_value"),
                    "controlled_verdict": comparison.get("verdict"),
                    "contribution": _comparison_contribution(comparison),
                    "hardware_used": comparison.get("hardware_used") is True,
                    "simulation_used": comparison.get("simulation_used") is True,
                    "fallback_declared": protocol.get("fallback")
        ...
        reviews.append(
            {
                ...
                "simulation_used": any(row.get("simulation_used") is True for row in empirical),
                "contribution": contribution,
                "contribution_label": contribution.replace("_", " ").title(),
                "classical_baseline": {
                    "name": next(
                        (
                    ...
                    "holdout_metric": representative.get("classical_holdout_metric"),
                },
                "quantum_or_nonlinear_result": {
                    "holdout_metric": representative.get("nonlinear_or_quantum_holdout_metric"),
                    "incremental_holdout_value": representative.get("incremental_holdout_value"),
                    "quantum_usefulness_score": representative.get("quantum_usefulness_score"),
                    "controlled_verdict": representative.get("verdict"),
                },
                "complexity_penalty": representative.get("complexity_penalty"),
                "latency_penalty": representative.get("latency_penalty"),
                "reliability": representative.get("reliability"),
                "net_usefulness": (
                    representative.get("quantum_usefulness_score")
                    if representative.get("quantum_usefulness_score") is not None
                    else representative.get("incremental_holdout_value")
                    if contribution == "incremental"
                    else None
                ),
                "overfit_audit": {
                ...
                "returned_to": "Pattern Recognition",
                "return_route": _route(
                    "patterns",
                    "findings",
                    "Pattern Recognition",
                ...
                "blocked_by": (
                    ...
                    if not empirical
                        ...
                        "No tested method earned incremental edge credit under the controlled verdicts."
                    ]
                    if contribution != "incremental"
                    else []
                ),
    ...
    return {
        ...
        "headline": (
            "A nonlinear or quantum method added controlled holdout value."
            if verdict_counts["nonlinear_strengthened"]
            else "No tested quantum method added reliable incremental value."
            if empirical_total
        ...
        "plain_english_summary": (
            ...
            else "Controlled comparisons are available below. Their explicit verdicts, rather than raw metric differences, determine whether quantum or nonlinear work added useful evidence."
        ),
        "purpose": (
        ...
        "current_method_state": {
            ...
            "simulator_completed_count": simulator_total,
            "fallback_declared_count": sum(
                row.get("fallback") == "deterministic_classical_shadow" for row in protocols
            ),
            "plain_english": (
                "No quantum hardware or simulator result has been recorded; the current records define classical fallback protocols only."
                if empirical_total == 0
                else f"{simulator_total} local-simulator comparisons and {hardware_total} physical-hardware comparisons are recorded. Method truth is shown on every completed comparison."
            ),
        },
        "funnel": [
            ...
            {
                "key": "strengthened",
                "label": "Nonlinear strengthened",
                "count": verdict_counts["nonlinear_strengthened"],
            },
            {
                "key": "returned",
                "label": "Verdicts returned",
                "count": sum(review["verdict"] != "not_measurable" for review in reviews),
            },
        ],
...
def validate_quantum_review(payload: dict[str, Any]) -> list[str]:
    ...
    for row in payload.get("reviews") or []:
        ...
        if row.get("contribution") not in {
            "incremental",
            "neutral",
            "fallback",
            "not_useful",
        }:
            errors.append(f"quantum_review_{review_id}_contribution_invalid")
        ...
        if (
            row.get("verdict") == "nonlinear_strengthened"
            and row.get("contribution") != "incremental"
        ):
            errors.append(f"quantum_review_{review_id}_strengthened_without_verdict_credit")
        errors.extend(validate_authority(row.get("authority", {}), prefix=f"quantum_{review_id}"))
    if (
        payload.get("empirical_comparison_count") == 0
        and payload.get("status") == "comparison_available"
    ):
