...
class ExecutionContext(StrictModel):
    ...
    def quote_fields_match_status(self) -> "ExecutionContext":
        if self.status == "quote_ready":
            ...
            if min(self.bid, self.ask, self.midpoint) <= 0:
                raise ValueError("quote_prices_must_be_positive")
        if datetime.fromisoformat(self.expires_at.replace("Z", "+00:00")) <= datetime.fromisoformat(self.observed_at.replace("Z", "+00:00")):
            raise ValueError("execution_context_expiry_not_after_observation")
        return self
