...
def classify_public_proof_state(
    *,
    ...
    if (
        ...
        return "validated"
    if scientific_verdict in {"quantum_strengthened", "joint_corroboration"} and empirical_measured:
        return "provisional"
    return "unproven"
...
def build_wave_h_certification(
    *,
    ...
    eligible_windows = _safe_int(alignment_truth.get("eligible_point_in_time_window_count"))
    ...
    for verdict in (
        ...
        if _safe_int(verdict_counts.get(verdict)) > 0:
            ...
            break
    empirical_measured = _safe_int(evaluation_summary.get("empirical_measured_count")) > 0
    ...
    payload: dict[str, Any] = {
        ...
        "plain_english_summary": (
            ...
            + (
                f"The historical spine now contains {eligible_windows} eligible evidence windows, "
                "but the untouched control suite and matched hardware comparison have not run. "
                if eligible_windows
                else "The available historical spine has no eligible untouched windows. "
            )
            + "No IBM hardware experiment was authorized or run, so the market result remains unproven."
        ),
        ...
        "evidence_truth": {
            ...
            "completed_partition_count": _safe_int(evidence_truth.get("completed_partition_count")),
            "provider_history_certified_complete": provider_complete,
            "leakage_violation_count": _safe_int(leakage_truth.get("leakage_violation_count")),
        },
        "engineering_fixture": {
            ...
            "hardware_smoke_manifest_hash": _text(hardware_public_state.get("manifest_hash")),
        ...
        "hardware_authorization_checkpoint": {
            ...
            "authorized": False,
            "engineering_manifest_hash": _text(hardware_public_state.get("manifest_hash")),
            ...
            "maximum_provider_budget_usd": _safe_dict(hardware_manifest.get("policy_contract")).get(
                "maximum_provider_budget_usd"
            ),
        ...
        "downstream_truth": {
            ...
            "paper_order_count": _safe_int(wave_g_integration.get("paper_order_created_count")),
...
def validate_wave_h_payload(payload: dict[str, Any]) -> list[str]:
    ...
    if fixture.get("contract_fixture_only") is True and payload.get("public_proof_state") in {
        "provisional",
        "validated",
    }:
    ...
    if (
        payload.get("expansion", {}).get("allowed") is True
        and payload.get("public_proof_state") != "validated"
    ):
    ...
    for family in ("engineering_checks", "scientific_checks"):
        for row in _safe_list(certification.get(family)):
            ...
            if (passed is True and status != "passed") or (passed is False and status == "passed"):
                errors.append(
                    f"wave_h_check_semantics_inconsistent:{_text(check.get('key'), 'unknown')}"
                )
    material = {
...
def build_current_wave_h_certification(
    *,
    ...
    return build_wave_h_certification(
        ...
        evaluation_summary=_read_json(runtime_dir / "qadam_independent_quantum_value_summary.json"),
