...
from orchestrator.decisions.draft_selection import (
    _candidate_identity as _candidate_identity, _draft_priority as _draft_priority,
    _rejection, select_drafts,
)
...
def _collect_drafts(runtime: Path) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    return select_drafts(source_rows, current_direction_ids, rejections)
...
def build_and_write_tradeability_pipeline(
    ...
    if read_json(project_root() / SCHEMA_PATH) != envelope_schema():
        errors.append("reviewed_tradeability_schema_missing_or_mismatched")
        state["checks"].update(status="blocked", validation_errors=errors,
                              validation_error_count=len(errors), implementation_ready=False)
