...
STAGES: tuple[dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "number": 6,
        "label": "Qadam Strategy Decision",
        "short_label": "Qadam Decision",
        "plain_english": "Qadam selects paper setups from current evidence; optional uncertainty reduces size.",
        "key_question": "Is this idea practical to trade now?",
        "sub_stages": [
            "Build complete practical context",
            "Identify the current entry trigger",
            "Check liquidity and invalidation",
            "Adjust conviction for optional evidence",
            "Return pass, hold, or veto",
        ],
        "inputs": ["Validated strategy or bounded discovery hypothesis", "Current execution evidence"],
        "outputs": ["Qadam selection, watchlist, hold or safety rejection with explanation"],
        "actors": ["Frontier LLM Strategy Lead", "Python COO"],
        "primary_routes": ["decide/decision"],
        "supporting_routes": ["decide/strategies"],
        "safety_boundary": "Qadam selection is not portfolio-risk approval, execution approval, or an order.",
    },
    {
        ...
        "inputs": ["Qadam strategy decision", "Risk state", "PaperOps safety state"],
...
ROUTE_CONTEXTS: dict[str, dict[str, Any]] = {
    ...
    "decide/decision": {
        ...
        "module_relationship": "This page owns Qadam's strategy decision, portfolio governance, the Router state, and the guarded PaperOps boundary.",
...
STAGE_ARTIFACT_REFS: dict[str, list[str]] = {
    ...
    "filter_tradeability": [
        "data/runtime/qadam_strategy_decision_results.jsonl",
        "data/runtime/qadam_strategy_decision_summary.json",
    ],
    "govern_decision": [
    ...
    "improve_reenter": [
        ...
        "data/runtime/qadam_stage1_learning_input.json",
    ],
}
...
def _stage_freshness(stage_id: str, freshness: dict[str, Any]) -> tuple[str, str | None]:
    ...
    timestamps = [
        str(record.get("generated_at")) for record in matched if record.get("generated_at")
    ]
...
def build_lifecycle_dashboard_summary(
    ...
    *,
    ...
    generated = generated_at or now_iso()
    source = (
        context.get("source_summary") if isinstance(context.get("source_summary"), dict) else {}
    )
    source_count = _safe_int(source.get("source_count"))
    fresh_source_count = _safe_int(source.get("fresh_count"))
    universe = (
        context.get("trading_universe") if isinstance(context.get("trading_universe"), dict) else {}
    )
    ...
    backfill = (
        context.get("backfill_progress")
        if isinstance(context.get("backfill_progress"), dict)
        else {}
    )
    ...
    quantum = (
        context.get("quantum_review") if isinstance(context.get("quantum_review"), dict) else {}
    )
    ...
    akber_veto = sum(
        count for key, count in akber_counts.items() if "veto" in key or "reject" in key
    )
    router = (
        context.get("router_scoreboard")
        if isinstance(context.get("router_scoreboard"), dict)
        else {}
    )
    ...
    router_why_not = (
        context.get("router_why_not") if isinstance(context.get("router_why_not"), dict) else {}
    )
    lifecycle = context.get("lifecycle") if isinstance(context.get("lifecycle"), dict) else {}
    order_count = _field_count(
        lifecycle,
        "broker_record_count",
        "order_record_count",
        "order_count",
    ...
    mirror_only_order_count = _safe_int(origin_counts.get("mirror_only_historical_record"))
    qadam_origin_order_count = sum(
        _safe_int(value)
        for key, value in origin_counts.items()
        if key != "mirror_only_historical_record"
    )
    actionable_order_count = qadam_origin_order_count if origin_counts else order_count
    ambiguous_count = _safe_int(lifecycle.get("ambiguous_lifecycle_count"))
    portfolio = (
        context.get("current_portfolio")
        if isinstance(context.get("current_portfolio"), dict)
        else {}
    )
    open_positions = _field_count(
        portfolio, "position_count", "open_position_count", "rows", "positions"
    )
    learning = (
        context.get("learning_cycle") if isinstance(context.get("learning_cycle"), dict) else {}
    )
    ...
    improvement = (
        context.get("improvement_pipeline")
        if isinstance(context.get("improvement_pipeline"), dict)
        else {}
    )
    improvement_counts = (
        improvement.get("counts") if isinstance(improvement.get("counts"), dict) else {}
    )
    ...
    source_state = (
        "unavailable"
        if not source_count
        else ("degraded" if fresh_source_count < source_count else "active")
    )
    source_blockers = (
        []
    ...
    evidence_state = (
        "active"
        if instrument_count and fresh_source_count
        else ("waiting_for_evidence" if instrument_count or source_count else "unavailable")
    )
    evidence_blockers = (
        []
        if instrument_count and fresh_source_count
        else ["Qualified source and market evidence is incomplete."]
    )
    pattern_state = (
        "active" if findings else ("waiting_for_evidence" if instrument_count else "unavailable")
    )
    strategy_state = "active" if hypotheses else ("waiting_for_evidence" if findings else "idle")
    edge_state = (
        "active"
        if validated_edges
        else ("waiting_for_evidence" if hypotheses or findings else "idle")
    )
    if akber_results:
        akber_state = (
            "active"
            if akber_pass
            else ("blocked" if akber_veto and not akber_hold else "waiting_for_evidence")
        )
    else:
        akber_state = "waiting_for_evidence" if hypotheses else "idle"
    governance_state = (
        "active"
        if paper_review_count or handoff_count
        else ("waiting_for_evidence" if router_decisions else "idle")
    )
    ...
    execution_state = (
        "degraded"
        if ambiguous_count
        else ("active" if actionable_order_count or open_positions else "idle")
    )
    learning_state = (
        "active"
        if learning_event_count or outcome_count
        else ("waiting_for_evidence" if actionable_order_count else "idle")
    )
    improvement_state = (
        "active"
        if active_improvements or applied_improvements
        else ("waiting_for_evidence" if learning_event_count else "idle")
    )
    ...
    stage_states = {
        "observe_world": _runtime_record(
            stage_id="observe_world",
            state=source_state,
            summary=f"{fresh_source_count} of {source_count} connected sources are currently fresh."
            if source_count
            else "Current source state is unavailable.",
        ...
        "form_strategy_hypotheses": _runtime_record(
            ...
            blockers=[]
            if hypotheses
            else ["No pattern has produced an eligible strategy hypothesis yet."],
            freshness=freshness,
        ...
        "filter_tradeability": _runtime_record(
            stage_id="filter_tradeability",
            state=akber_state,
            summary=f"Qadam has {akber_pass} selections, {akber_hold} holds, and {akber_veto} unsafe-evidence vetoes in the current evidence set.",
            item_count=len(akber_results),
            blockers=[]
            if akber_pass
            else ["No idea currently has a complete practical tradeability pass."],
            freshness=freshness,
            metrics=[
        ...
        "govern_decision": _runtime_record(
            ...
            if paper_review_count or handoff_count
                ...
                str(
                    ...
                    or "No setup is currently eligible for guarded paper review."
                )
            ],
            freshness=freshness,
            metrics=[
        ...
        "execute_monitor": _runtime_record(
            ...
            summary=(
                f"{mirror_only_order_count} broker-mirror records are historical reference only; "
                f"{qadam_origin_order_count} Qadam-origin lifecycle records and "
                f"{open_positions} open positions are visible. {ambiguous_count} lifecycle "
                "states are ambiguous."
                if origin_counts
                else f"{order_count} paper-order lifecycle records and {open_positions} open positions are visible; {ambiguous_count} lifecycle states are ambiguous."
            ),
            item_count=order_count + open_positions,
            blockers=[]
            if not ambiguous_count
            else [f"{ambiguous_count} paper-order lifecycle states need reconciliation."],
            freshness=freshness,
            metrics=[
                {"label": "Broker mirror records", "value": order_count},
                {"label": "Mirror-only historical", "value": mirror_only_order_count},
                {"label": "Qadam-origin lifecycle", "value": qadam_origin_order_count},
        ...
        "learn_outcome": _runtime_record(
            ...
            else ["No attributable Qadam outcome is ready for a supported lesson yet."],
            freshness=freshness,
            metrics=[
        ...
        "improve_reenter": _runtime_record(
            ...
            blockers=[]
            if active_improvements or applied_improvements
            else ["No supported lesson has opened a measurable improvement test."],
            freshness=freshness,
            metrics=[
...
def validate_lifecycle_contract(contract: dict[str, Any]) -> list[str]:
    ...
    route_contexts = (
        contract.get("route_contexts") if isinstance(contract.get("route_contexts"), dict) else {}
    )
