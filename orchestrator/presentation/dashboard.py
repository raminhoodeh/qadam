...
def build_dashboard_view_model(settings: Settings | None = None) -> dict[str, Any]:
    ...
    payload["operating_picture"] = build_picture(
        ...
        capability=_read_json(runtime / "qadam_source_capability_registry.json"),
...
def load_dashboard_view_model(settings: Settings | None = None) -> dict[str, Any]:
    ...
    generation = read_projection(runtime)
    documents = generation[0] if generation else None

    def read(name):
        return documents.get(name, {}) if documents is not None else _read_json(runtime / name)

    status = dict(read(STATUS_ARTIFACT))
    if generation:
        status["source_generation_id"] = generation[1]
    if not status:
        return {}
    status["decision_records"] = read(DECISION_RECORDS_ARTIFACT)
    status["system_map"] = read(SYSTEM_MAP_ARTIFACT)
    status["portfolio_value"] = read(PORTFOLIO_SERIES_ARTIFACT)
    status["current_portfolio"] = read(CURRENT_PORTFOLIO_ARTIFACT)
    status["trading_history"] = read(TRADING_HISTORY_ARTIFACT)
    status["source_network"] = read(SOURCE_NETWORK_ARTIFACT)
    status["strategy_universe"] = read(STRATEGY_UNIVERSE_ARTIFACT)
    status["pattern_lab"] = read(PATTERN_LAB_ARTIFACT)
    status["evidence_quality"] = read(EVIDENCE_QUALITY_ARTIFACT)
    status["trade_intents"] = read(TRADE_INTENTS_ARTIFACT)
    status["pattern_to_paper_workflow"] = read(PATTERN_TO_PAPER_WORKFLOW_ARTIFACT)
    status["pattern_intelligence"] = read(PATTERN_INTELLIGENCE_ARTIFACT)
    status["learning_ledger"] = read(LEARNING_LEDGER_ARTIFACT)
    status["repair_queue"] = read(REPAIR_QUEUE_ARTIFACT)
    # Telegram transport exports have their own owner and are not dashboard members.
    ...
    status["anti_slop_audit"] = read(ANTI_SLOP_ARTIFACT)
    return status
...
def write_dashboard_view_model(
    ...
    *,
    ...
    documents = {paths[key].name: payload[key] for key in paths if key not in {"status", "anti_slop", "phase_status"}}
    documents[paths["status"].name] = _status_summary(payload)
    documents[paths["anti_slop"].name] = payload["anti_slop_audit"]
    documents[paths["phase_status"].name] = build_qsase_phase_implementation_status(payload)
    publication = publish_projection(runtime, documents)
    written = {key: str(path) for key, path in paths.items()}
    if append_history and publication["changed_document_count"]:
...
def build_and_write_dashboard_view_model(
    ...
    if errors:
        # Keep the last coherent generation available without presenting a
        # failed rebuild as a successful check or refreshing evidence times.
        _write_json(_runtime_dir(settings) / "qadam_dashboard_projection_attempt.json", {
            "generated_at": payload.get("generated_at"), "status": "failed",
            "validation_errors": errors, "previous_generation_retained": True,
        })
        return payload, {}, errors
    written = write_dashboard_view_model(payload, settings)
    _write_json(_runtime_dir(settings) / "qadam_dashboard_projection_attempt.json", {
        "generated_at": payload.get("generated_at"), "status": "passed",
        "validation_errors": [], "previous_generation_retained": False,
    })
