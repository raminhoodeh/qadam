...
1. Portfolio: performance, allocation, risk, and positions.
2. Timeline.
3. Source Intelligence Network.
4. Trading Strategy Universe.
5. Pattern & Opportunity Lab.
6. Decision Room: Current Decision first, then Decisions Brewing (trade intents).

That order is deliberate: portfolio first, then chronology, intelligence, strategy, and current trade decisions. The dashboard remains an oversight surface, not order entry.
...
- `scripts/check_strategy_lead_durable_context.py` validates that Strategy Lead packets receive sanitized durable replay context, create challenge questions, and keep risk handoff, trade-candidate creation, orders, broker writes, and live capital disabled.
...
- Strategy Lead shadow packets now include sanitized source/replay context and a challenge-only strategy review: source posture, evidence pressure, replay counts, required challenges, paper context state, and explicit zero authority for risk handoff, trade candidates, orders, broker writes, and live capital.
...
- `orchestrator/quantum.py` now starts Phase 3 with a hardened Head of Quant quantum/classical oracle path. It defines provider readiness, `QuantumBackend`, optional Qiskit Aer local backend, deterministic classical fallback, `QuantumOracleJob`, `QuantumOracleResult`, local circuit blueprint, measurement counts, stable input fingerprint, weekly cadence metadata, local validation, and a JSONL result store.
- `scripts/check_quantum_oracle.py` validates Pattern Recognition and Strategy Collapse / Ambiguity Score jobs. The current runtime uses `classical_fallback` because local `qiskit`/`qiskit-aer` packages are not installed, and keeps hardware scheduler enablement, hardware submissions, execution approvals, paper-order approvals, broker writes, and trade-candidate creation at zero.
...
- `landing-page-repo/dashboard.js` now tries the authenticated `/api/cockpit-status` live bridge first, then falls back to `/status/cockpit-status.json`, and renders the QSASE public dashboard when `qsase_dashboard` is present: a consolidated Portfolio page for performance, allocation, risk, and open positions; a separate Timeline; source network; strategy universe; pattern lab; Decision Room; Router/PaperOps state; and explicit zero-authority flags.
...
- The cockpit cognition contract can now include sanitized Phase 2 durable replay cycle state, local Research Analyst assessments, Strategy Lead challenge-only replay context, read-only paper-account context, and Signal Integrity review state: replay mode, replayed source count, queued packet count, source posture, required challenges, summary, watch focus, missing correlations, next questions, confidence, paper mirror state, integrity score, Akber filter output, failure reasons, required next steps, and shadow-only authority flags.
...
- The cockpit status contract now includes sanitized Head of Quant oracle state: backend, local simulation mode, latest recommendation, result count, cadence, fingerprint, validation checks, optional `qiskit`/`qiskit-aer` availability, hardware-submission counts, and zero-authority flags for execution, paper orders, trade-candidate creation, hardware scheduler enablement, and hardware jobs.
- Telegram provides outbound member updates, read-only research intake, and group-scoped read-only queries for status, portfolio, trading, patterns, health, and repairs. It has no execution, approval, repair, broker, proof, code, secret, or live-capital authority.
...
6. Run `scripts/check_strategy_lead_durable_context.py` to prove Strategy Lead consumes replay context as challenge-only analysis.
7. Use `scripts/start_local_stores.sh` later when Chroma server mode is needed as well.
