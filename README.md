...
- Dashboard Plan D5 is implemented locally: `orchestrator/trade_intent.py`, `orchestrator/risk_agent.py`, `orchestrator/execution_policy.py`, `orchestrator/staged_paper_order.py`, `scripts/check_trade_intent.py`, `scripts/check_risk_agent_policy_router.py`, `scripts/check_execution_policy_router.py`, and `scripts/check_staged_paper_order_contract.py` create a local Trade Intent Store plus read-only Risk Agent, Execution Policy, and disabled staged paper-order review layers. The cockpit renders one candidate, one blocked D5 test intent, current Risk Agent policy reviews, current Execution Policy / kill-switch reviews, and disabled staged paper-order reviews from those stores.
