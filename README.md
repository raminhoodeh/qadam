...
- Dashboard Plan D5 is implemented locally: `orchestrator/trade_intent.py`, `orchestrator/risk_agent.py`, `orchestrator/execution_policy.py`, `scripts/check_trade_intent.py`, `scripts/check_risk_agent_policy_router.py`, and `scripts/check_execution_policy_router.py` create a local Trade Intent Store plus read-only Risk Agent and Execution Policy review layers. The cockpit renders one candidate, one blocked D5 test intent, current Risk Agent policy reviews, and current Execution Policy / kill-switch reviews from those stores.
