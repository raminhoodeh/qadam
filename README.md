...
- Dashboard Plan D5 is implemented locally: `orchestrator/trade_intent.py`, `orchestrator/risk_agent.py`, `scripts/check_trade_intent.py`, and `scripts/check_risk_agent_policy_router.py` create a local Trade Intent Store plus a read-only Risk Agent policy-review layer. The cockpit renders one candidate, one blocked D5 test intent, and current Risk Agent policy reviews from those stores.
